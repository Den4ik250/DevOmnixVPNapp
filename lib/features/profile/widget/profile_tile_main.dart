...
import 'package:devomnix/features/profile/notifier/profile_notifier.dart';
...
class ProfileTileMain extends HookConsumerWidget {
                          ...
                          if (subInfo.webPageUrl != null)
                            Expanded(
                              child: InkWell(
                                ...
                                borderRadius: BorderRadius.circular(8),
                                child: _InfoItem(
                                  iconWidget: _getLinkIconWidget(subInfo.webPageUrl!, FluentIcons.building_shop_24_regular),
                          ...
                          if (subInfo.supportUrl != null) ...[
                            ...
                            Expanded(
                              child: InkWell(
                                ...
                                borderRadius: BorderRadius.circular(8),
                                child: _InfoItem(
                                  iconWidget: _getLinkIconWidget(subInfo.supportUrl!, null),
  ...
  Widget _getLinkIconWidget(String url, [IconData? fallback]) {
    final uri = Uri.parse(url);
    final host = uri.host.toLowerCase();
    final size = 20.0;

    if (host.endsWith('telegram.me') || host.endsWith('t.me')) {
      return FaIcon(FontAwesomeIcons.telegram, size: size);
    }
    ...
    return Icon(fallback ?? FluentIcons.link_24_regular, size: size);
  }
  ...
  String _formatSupportLink(String url) {
    ...
    if (host.endsWith('hiddify.com') || host.endsWith('devomnix.com')) {
      return "DevOmnix";
    }
    return uri.host;
  }
...
class _InfoItem extends StatelessWidget {
  const _InfoItem({required this.iconWidget, required this.label, required this.value});
